"""
Create a synthetic highway clip in RDS-HQ format, seen from a fixed camera (pole, gantry, car parked on the
shoulder) or from an ego car driving or stopped in a lane. Vehicles drive through the scene on a straight
multi-lane highway, optionally changing lanes and zigzagging (see traffic_simulation.py). The output can be rendered with
...
from traffic_simulation import simulate
...
def lane_name(lane, num_lanes):
    if num_lanes % 2 == 1 and lane == num_lanes // 2:
        return "middle lane"
    if lane == 0:
        return "left lane, next to the median"
    if lane == num_lanes - 1:
        return "right lane"
    return f"lane {lane + 1} of {num_lanes}, counted from the median"
...
def make_captions(num_lanes, cam_y, cam_height, yaw, median_width, lane_width, min_gap, max_gap, min_speed,
    ...
    camera_static = ego_lane is None or ego_speed <= 0.1
    if ego_lane is not None:
        where = f"in the {lane_name(ego_lane, num_lanes)} of a highway"
        if camera_static:
            intro = (f"The video is captured by the front camera of a car standing still {where}. The car is stopped, "
                     f"so the camera does not move, while vehicles going the same direction pass on both sides.")
        else:
            intro = (f"The video is captured by the front camera of a car driving at about {ego_speed * 3.6:.0f} km/h "
                     f"{where}, in the same direction as the surrounding traffic. The camera moves forward smoothly "
                     f"with the car.")
    elif abs(cam_y) > half_road and cam_height < 4.0:
    ...
    if camera_static:
        base += (" The camera is completely static: the road, the lane markings, the poles and the signs stay fixed "
                 "in the frame, and only the vehicles move.")
...
@click.option("--ego_lane", type=str, default=None,
              help="camera on an ego car in this lane of the +x carriageway: 'middle' or a lane index "
...
def main(parked, facing, ego_lane, **kwargs):
    if parked and ego_lane is not None:
        raise click.UsageError("use either --parked or --ego_lane")
    if ego_lane is not None:
        ego_lane = kwargs["num_lanes"] // 2 if ego_lane == "middle" else int(ego_lane)
        if not 0 <= ego_lane < kwargs["num_lanes"]:
            raise click.BadParameter(f"--ego_lane must be 'middle' or 0..{kwargs['num_lanes'] - 1}")
    kwargs["ego_lane"] = ego_lane
    if parked:
        defaults = {"shoulder_width": 3.5, "cam_height": 1.5, "pitch": 1.5, "hfov": 100.0}
    elif ego_lane is not None:
        defaults = {"shoulder_width": 0.5, "cam_height": 1.5, "pitch": 1.5, "hfov": 100.0}
    else:
        defaults = {"shoulder_width": 0.5, "cam_height": 8.0, "pitch": 12.0, "hfov": 60.0}
...
def create_scene(output_root, clip_id, num_frames=121, num_lanes=3, lane_width=3.6, median_width=2.0,
    ...
    driving at ego_speed m/s (0 = standing still); cam_x is where the ego car is at the first frame.
    ...
    # 2. camera: fixed, or riding on the ego car (front camera 1 m ahead of the car center)
    pose_sample = {'__key__': clip_id}
    for frame_idx in range(num_frames):
        if ego_path is None:
            camera_to_world = look_at_camera_to_world(np.array([cam_x, cam_y, cam_height]), yaw, pitch)
        else:
            ego_x, ego_y, ego_yaw = ego_path[frame_idx]
            position = np.array([ego_x + np.cos(ego_yaw), ego_y + np.sin(ego_yaw), cam_height])
            camera_to_world = look_at_camera_to_world(position, np.rad2deg(ego_yaw), pitch)
    ...
    # 4. moving vehicles, one entry per frame
    ...
    for frame_idx in range(num_frames):
        ...
        for v in vehicles:
            if sim_frames is None:
                x, y, heading, vx, vy = v["x0"] + v["velocity"] * t, v["y"], v["yaw"], v["velocity"], 0.0
            elif v["track_id"] in sim_frames[frame_idx]:
                x, y, heading, vx, vy = sim_frames[frame_idx][v["track_id"]]
            else:
                continue  # removed to make room for the ego car
            if not (x_min <= x <= x_max):
                continue
            frame_objects[v["track_id"]] = {
                'object_to_world': object_to_world(x, y, v["lwh"][2] / 2, heading).tolist(),
                'object_lwh': v["lwh"],
                'object_is_moving': bool(np.hypot(vx, vy) > 0.2),
                'object_type': v["type"],
                'object_subtype': v["subtype"],
                'object_velocity': [float(vx), float(vy), 0.0],  # world frame, m/s
            }
            if "towed_by" in v:
